...
var (
	tZeroer = reflect.TypeOf((*bson.Zeroer)(nil)).Elem()
)
...
func lookupElementEncoder(ec bson.EncodeContext, origEncoder bson.ValueEncoder, currVal reflect.Value) (bson.ValueEncoder, reflect.Value, error) {
...
Special thanks to the MongoDB Go Driver v2 team for sealing every door and
leaving exactly one window. It was very inspiring. 🙃
*/
//
/*
...
func (sc *PreserveStructCodec) EncodeValue(ec bson.EncodeContext, vw bson.ValueWriter, val reflect.Value) error {
	...
	sd, err := sc.describeStruct(ec.Registry, val.Type(), sc.UseJSONStructTags, sc.ErrorOnInlineDuplicates)
	if err != nil {
		return err
	}
	...
	for _, desc := range sd.fl {
		...
		if rv.Kind() == reflect.Interface {
			...
			empty = isEmpty(rv, sc.encodeOmitDefaultStruct || sc.OmitZeroStruct)
		}
...
func (sc *PreserveStructCodec) DecodeValue(dc bson.DecodeContext, vr bson.ValueReader, val reflect.Value) error {
	...
	case bson.Type(0), bson.TypeEmbeddedDocument:
	case bson.TypeNull:
	...
	sd, err := sc.describeStruct(dc.Registry, val.Type(), sc.UseJSONStructTags, false)
	if err != nil {
		return err
	}

	if sc.decodeZeroStruct || sc.ZeroStructs {
		val.Set(reflect.Zero(val.Type()))
	}
...
func isEmpty(v reflect.Value, omitZeroStruct bool) bool {
	...
	if (kind != reflect.Ptr || !v.IsNil()) && v.Type().Implements(tZeroer) {
		return v.Interface().(bson.Zeroer).IsZero()
	}
	switch kind {
...
func (sc *PreserveStructCodec) describeStructSlow(
	...
	for i := 0; i < numFields; i++ {
		...
		if useJSONStructTags {
			stags, err = x.ParseJSONStructTags(sf)
		} else {
			stags, err = x.ParseStructTags(sf)
		}
		if err != nil {
			return nil, err
		}
